...
def upload_image():
    ...
    if operation_selection == 'gray':
        ...
        print('please select an option from the dropdown to continue- image type not selected')
...
def invert(decode_array_to_img): 
    invert_ef= cv2.bitwise_not(decode_array_to_img)
    status, output_img= cv2.imencode('.PNG', invert_ef)
